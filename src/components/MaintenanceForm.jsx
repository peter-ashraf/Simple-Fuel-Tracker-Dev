...
import { Plus, Wrench, CalendarBlank, Tag, CaretLeft, Shield, CaretRight, FloppyDisk, Bell } from '@phosphor-icons/react';
        ...
        <div className="fixed-button-container maintenance-form-actions">
          <div className="max-w-lg mx-auto flex gap-3">
            ...
            <button type="button" onClick={handleSubmit} disabled={!performedAtODO || !intervalKm} className="flex-[2] px-6 bg-emerald-500 text-white dark:text-slate-950 font-bold h-[64px] rounded-[1.5rem] flex items-center justify-center gap-2 transition-all">
              <FloppyDisk weight="duotone" className="w-5 h-5" /> <span>Save Service</span>
            </button>
          </div>
        </div>,
        document.body
            ...
            <Card className="p-6">
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <Input type="number" inputMode="numeric" value={performedAtODO} onChange={(e) => setPerformedAtODO(e.target.value)} placeholder={t('current_mileage')} min="0" required />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label className="flex items-center gap-2"><Wrench weight="duotone" className="w-4 h-4" /> {t('distance')} (km) *</Label>
                    <Input type="number" inputMode="numeric" value={intervalKm} onChange={(e) => setIntervalKm(e.target.value)} placeholder="e.g. 10000" min="1" required />
                  </div>
                  <div>
                    <Label className="flex items-center gap-2"><Shield weight="duotone" className="w-4 h-4" /> {t('safety_margin')}</Label>
                    <Input type="number" inputMode="numeric" value={safetyMarginKm} onChange={(e) => setSafetyMarginKm(e.target.value)} placeholder="2000" min="0" />
                  </div>
                </div>
                <div>
